...
{%- endif %}
  ...
  processor:
    ...
    # AttributeFilter: 属性过滤处理器（嘉为）
    - name: "attribute_filter/canway"
      config:
        as_string:
          keys:
            - "attributes.db.name"
        ...
        assemble:
          - destination: "db.system"
            predicate_key: "attributes.db.system"
            default_from: "attributes.db.system"
            rules:
              - kind: ""
                first_upper:
    ...
    # Probe_filter 探针采集过滤器 探针自定义配置下发（嘉为）
    - name: "probe_filter/common"

    # TokenReplacer: Token 替换器（token 映射）（嘉为）
